...
class Product extends AbstractClasses
{
    ...
    public function deleteProduct(int $producttoken): bool
    {
        try {
           ...
           echo  $imageNames = $stmt->fetch(PDO::FETCH_COLUMN);
        //    exit; 

            # Delete all images from server
                    $target_file = $_SERVER['DOCUMENT_ROOT'] . '/solar/uploads/' . $imageNames;
    ...
    #getAllBuyBackProduct::This method checks for all products uploaded by Admin
    ...
    public function getAllProductPurchasedInstallmentally()
    {
        ...
        $sql = "SELECT COUNT(id) AS productCount FROM tbl_installment_purchases";
        $stmt = $this->conn->prepare($sql);
    ...
    public function searchProducts($searchItem)
{
    try {
        ...
        $sql = "SELECT * FROM tblproducts WHERE $searchConditions ";
        $stmt = $this->conn->prepare($sql);
