...
class Loan extends AbstractClasses
{
    ...
    public function calculateMonthsAheadAndSave(int $subscribedPackage, int $usertoken, int $loantoken, string $repaymentPerMonth)
    {
        ...
        for ($i = 0; $i < $subscribedPackage; $i++) {
            ...
            $saveWhenExpectedToPay = $this->saveWhenExpectedToPay(
                $monthExpectedToPay,
                $usertoken,
                $loantoken,
                ceil($repaymentPerMonth),
                $remindADayBefore,
                $remindAWeekBefore,
                $remindThreeDaysBefore
            );
    ...
    public function saveWhenExpectedToPay(
        $monthExpectedToPay,
        int $usertoken,
        int $loantoken,
        string $repaymentPerMonth,
        $remindADayBefore,
        $remindAWeekBefore,
        $remindThreeDaysBefore
    ) {
